...
export async function ownerVerifyStoreDomain(storeId: string): Promise<{ verified: boolean; cnameTarget: string | null; expectedTarget: string | null; domain: string; reason: string }> {
  return apiFetch<{ verified: boolean; cnameTarget: string | null; expectedTarget: string | null; domain: string; reason: string }>(`/owner/stores/${storeId}/domain/verify`, {
    method: 'POST',
  });
}
...
export async function adminUploadStoreHeaderImage(storeId: string, file: File): Promise<{ url: string; store: Store }> {
  const formData = new FormData();
  formData.append('image', file);
  const res = await fetch(`/api/admin/stores/${storeId}/header-image`, {
    method: 'POST',
    credentials: 'include',
    body: formData,
  });
...
export async function adminUploadStorePhoto(storeId: string, file: File): Promise<{ url: string; storePhotos: string[]; store: Store }> {
  const formData = new FormData();
  formData.append('image', file);
  const res = await fetch(`/api/admin/stores/${storeId}/photos`, {
    method: 'POST',
    credentials: 'include',
    body: formData,
  });
...
export async function adminDeleteStorePhoto(storeId: string, photoIndex: number): Promise<{ storePhotos: string[]; store: Store }> {
